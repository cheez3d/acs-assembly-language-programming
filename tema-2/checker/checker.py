...
for n in range(1, numTasks + 1):
    ...
    for t in range(1, 6):
        ...
        if n in tasksWithARGV:
            with open('checker/inputs/task' + str(n) + "_" + str(t) + "_args") as f:
                args = f.read()
        proc = os.popen(runExec + " checker/inputs/task" + str(n) + "_" + str(t) + ".pgm " + 
                        str(n) + " " + args)
        result = proc.read().strip()
        proc.close()

        expectedResult = open("checker/outputs/task" + str(n) + "_" + str(t) + ".ref").read().strip()
